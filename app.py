...
def init_db():
    conn = sqlite3.connect('suivi_houbad_v7.db')
    c = conn.cursor()
    ...
    # Vérifier si la table est vide pour ne pas doubler les anciennes valeurs
...
def charger_data():
    conn = sqlite3.connect('suivi_houbad_v7.db')
    # On affiche l'ID dans le tableau comme demandé
...
with col2:
    ...
    if not df_mesures.empty:
        # On reformatte juste pour l'affichage
        ...
        with st.expander("🗑️ Supprimer par ID"):
